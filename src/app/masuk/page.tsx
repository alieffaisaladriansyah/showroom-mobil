...
export default function HalamanMasuk() {
  ...
  const tanganiMasuk = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // REDIRECT AGRESIF: Memaksa browser reload penuh agar cookies dibaca middleware
      window.location.href = "/admin/mobil";
    } catch (err) {
      const pesan = err instanceof Error ? err.message : "Terjadi kegagalan proses masuk.";
      setPesanEror(pesan);
      setLoading(false);
    }
  };
      ...
      <article className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
        <header className="text-center space-y-2">
          <div className="inline-flex items-center gap-2 text-blue-500 font-bold text-2xl tracking-tight justify-center">
            ...
            <span>DRIVE<span className="text-white">CORE</span></span>
          </div>
          <h1 className="text-xl font-extrabold text-white tracking-tight">Login Portal Admin</h1>
          <p className="text-xs text-slate-400">Gunakan akun admin yang terdaftar untuk mengelola inventaris showroom</p>
        </header>

        {pesanEror && (
          <div className="bg-red-950/40 border border-red-900 text-red-400 p-3 rounded-xl flex items-center gap-2 text-xs font-medium" role="alert">
        ...
        <form onSubmit={tanganiMasuk} className="space-y-4">
          <div className="space-y-1">
            <label htmlFor="email" className="text-xs font-bold uppercase tracking-wider text-slate-400">Alamat Email</label>
            <input
              id="email"
              type="email"
              required
          ...
          <div className="space-y-1">
            <label htmlFor="sandi" className="text-xs font-bold uppercase tracking-wider text-slate-400">Kata Sandi</label>
            <input
              id="sandi"
              type="password"
              required
              placeholder="••••••••"
              value={kataSandi}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setKataSandi(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 transition-all"
            />
          </div>
        ...
        <footer className="text-center text-xs text-slate-500 border-t border-slate-800 pt-4">
          ...
          <Link href="/daftar" className="text-blue-500 font-bold hover:underline">
            Daftar Di Sini
          </Link>
        </footer>
      </article>
